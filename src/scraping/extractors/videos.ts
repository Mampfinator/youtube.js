...
import { ScrapedShort, ScrapedStream, ScrapedVideo, StreamStatus, VideoType } from "../types/external/videos";
import { GridVideoRenderer, ItemReelItemRenderer } from "../types/internal/generated";
...
export function extractStream(renderer: GridVideoRenderer): ScrapedStream {
    ...
    const status: StreamStatus | undefined = STATUS_LOOKUP[overlay.style as "DEFAULT" | "LIVE" | "UPCOMING"];
    
    if (!status) throw new Error(`Could not determine stream status for overlay style ${overlay.style}`);
