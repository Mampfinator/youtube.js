...
import { AttachmentType, CommunityPost, PollChoice } from "../types/external/community-posts";

// TODO: type properly; figure out where quicktype put shared posts (if it even put them anywhere)
export function extractCommunityPost(rawPost: Record<string, any> ): CommunityPost {
    ...
    const post: CommunityPost & Record<string, any> = {id, content, attachmentType} as any;
    ...
    else if (post.attachmentType === AttachmentType.SharedPost) post.sharedPost = extractCommunityPost(originalPost.backstagePostRenderer);
