...
export abstract class ChannelTabContext<
    TExtractData extends {ytInitialData: YtInitialData} = {ytInitialData: YtInitialData}
> extends ScrapingContext<TExtractData> {
    ...
    private getTabData(): Result<TabData, Error> {
        ...
        try {
            const {tabs} = this.data.ytInitialData.contents.twoColumnBrowseResultsRenderer!;
...
function getTabName(path: string): Result<ChannelTab, Error> {
    const tab = path.trim().match(/(?<=\/)[a-z]+$/)?.[0]?.trim() as ChannelTab;
     if (!tab) return err(new Error(`Could not extract tab name from ${path}.`));
    if (!IS_CHANNEL_TAB_LOOKUP.has(tab)) return err(new Error(`Unknown tab ${tab} in ${path}`));

    return ok(tab);
}

class TabData<TTab extends ChannelTab = any> {
    public static from(rawTabs: TwoColumnBrowseResultsRenderer["tabs"]): Result<TabData, Error> {
        try {
            ...
            for (const tab of rawTabs) {
                ...
                if (!endpoint) return err(new Error("Could not extract endpoint for tab."));
                ...
                tabs[tabName.value] = {endpoint, selected, content: (selected ? (renderer as TabRenderer).content : undefined) as any};
            }

            if (!active) return err(new Error("Failed determining active tab."));
