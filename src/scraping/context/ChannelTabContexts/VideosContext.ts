...
export class VideosContext extends Mixin(
    ...
    ElementContext<ScrapedVideo>,
        ...
        private toVideos(renderers: (GridVideoRenderer | ItemVideoRenderer | PurpleVideoRenderer)[]): Map<string, ScrapedVideo> {
    ...
    > {
        ...
        if (data.isErr() || !data.value) {
            yield err([(data as Err<any, any>).error ?? new Error("Empty data!")]);
            return;
        }
        ...
        yield ok({
            elements: this.toVideos(renderers.map(({richItemRenderer}) => richItemRenderer?.content.videoRenderer!).filter(i => i))
        });
        ...
        try {
            while(token) {
                ...
                const items = continuation.value.onResponseReceivedActions!.map(({appendContinuationItemsAction}) => appendContinuationItemsAction?.continuationItems).filter(i => i).flat();

                yield ok({
                    elements: this.toVideos(items.map(item => item.richItemRenderer?.content.videoRenderer).filter( i => i))
                });
