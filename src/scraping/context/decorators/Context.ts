...
export type ContextData = {matcher: Matcher, constructor: Type<unknown>, weight: number};
...
export function Context(matcherOrRegex: RegExp | Matcher, weight?: number): ClassDecorator {
    ...
    return (target) => {
        ...
        Reflect.defineMetadata(CONTEXT_META_WEIGHT_KEY, weight ?? DEFAULT_WEIGHT, target);
        registered.add(target as unknown as Type<unknown>);
    }
}


export function getContextInfo<T extends Function>(target: T): [Matcher, number] {
    return [Reflect.getMetadata(CONTEXT_META_KEY, target), Reflect.getMetadata(CONTEXT_META_WEIGHT_KEY, target)];
}