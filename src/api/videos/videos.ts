...
type VideoListResponse = ListResponse<
...
export class VideosEndpoints extends YouTubeEndpoints {
    constructor(client: YouTubeClient) {
        super(client, "videos", [
            {
                for: "youtube#video",
                transform(source: VideoResource): Video<VideoPart> {
                    ...
                    if (source.liveStreamingDetails) {
                        entries.push([
                            ...
                            mapProperties(source.liveStreamingDetails, {
                                scheduledEndTime: toDateOpt,
                                actualEndTime: toDateOpt,
                                scheduledStartTime: toDateOpt,
                                actualStartTime: toDateOpt,
                            }),
                        ]);
                    }
                    ...
                    if (source.contentDetails) {
                        entries.push([
                            "contentDetails",
                            mapProperties(source.contentDetails, {
                                caption: Boolean,
                            }),
                        ]);
                    }
    ...
    public async list<TPart extends VideoPart>(
        options: ListOptions<TPart>,
    ): Promise<Result<Video<TPart>[], YouTubeAPIError | YouTubeClientError>> {
        ...
        const transformed = result.value.items.map(item =>
            this.transform<VideoResource, Video<VideoPart>>(item),
        );

        if (transformed.filter(v => v).length !== transformed.length)
            return err(new YouTubeClientError());

        return ok(transformed as Video<TPart>[]);
    }
}
