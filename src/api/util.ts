 ...
 * ```
 */
export function mapProperties<TSource extends object, TMapper extends { [P in keyof TSource]?: (input: TSource[P]) => any }>(source: TSource, mapper: TMapper): 
    { [P in keyof TSource]: TMapper[P] extends (...args: any[]) => any ? ReturnType<TMapper[P]> : TSource[P] } 
{
    return Object.fromEntries(
        Object.entries(source).map(([key, value]) => [
            key, key in mapper ? mapper[key as keyof TSource]!(value) : value
        ])
    ) as any;
}


export function clearRecord<TSource extends { [key: string]: any | undefined }>(source: TSource): { [P in keyof TSource]: TSource[P] extends undefined ? never : TSource[P] } {
    return Object.fromEntries(
        Object.entries(source).filter(([_, value]) => typeof value !== "undefined")
    ) as any;
}

export function deepClearRecord<TSource extends Record<string, Record<string, any | undefined>>>(source: TSource) {
    return Object.fromEntries(
        Object.entries(
            source
