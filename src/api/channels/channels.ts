...
type ChannelListResponse = ListResponse<
...
export class ChannelsEndpoints extends YouTubeEndpoints {
        ...
        super(client, "channels", [
            {
                ...
                transform(source: ChannelResource): Channel<ChannelPart> {
                    const replacers = {
                        snippet: (raw: ChannelResource["snippet"]) =>
                            mapProperties(source.snippet, {
                                publishedAt: toDate,
                            }),
                        contentOwnerDetails: (
                            raw: ChannelResource["contentOwnerDetails"],
                        ) =>
                            mapProperties(source.contentOwnerDetails, {
                                timeLinked: toDate,
                            }),
                    };

                    const entries: Entries<Channel<ChannelPart>> =
                        Object.entries(source).map(
                            ([key, value]: [any, any]) => [
                                key,
                                replacers[key as keyof typeof replacers]?.(
                                    value,
                                ) ?? value,
                            ],
                        );
    ...
    public async list<TPart extends ChannelPart>(
        options: ListOptions<TPart>,
    ): Promise<Result<Channel<TPart>[], YouTubeAPIError | YouTubeClientError>> {
        const {
            part,
            hl,
            maxResults,
        ...
        const transformed = result.value.items.map(item =>
            this.transform<ChannelResource, Channel<TPart>>(item),
        );
        if (transformed.filter(c => c).length !== transformed.length)
            return err(new YouTubeClientError());

        return ok(transformed as Channel<TPart>[]);
    }
}
