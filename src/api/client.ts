...
export class YouTubeClient extends Client {
    ...
    protected async fetch<T extends object>(
        options: FetchOptions,
    ): Promise<Result<T, YouTubeAPIError>> {
        const response = await this.request({
            ...
            path: `/${join(
                "youtube",
                "v3" /* should be a constant, but YouTube will never update their API anyway- */,
                options.path,
            )}`,
        });
    ...
    public async get<T extends object>(
        ...
        return this.fetch({
            ...
            method: "GET",
            ...deepClearRecord(
                (options as Record<string, Record<string, any>>) ?? {},
            ), // this is a bodge, but it works for now
        });
    }
}
