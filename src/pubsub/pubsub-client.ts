...
function verifyPostMessage(
    ...
    if (secret) {
        ...
        try {
            ...
            if (signature !== computedSignature) return {valid: false, status: 204};
...
export class PubsubListener {  
    ...
    public getGetListener(): Callback {
        return (req: ExpressRequest, res: ExpressResponse) => {
            ...
            if (!topic || typeof topic !== "string" || !mode || typeof topic !== "string" || !challenge || typeof challenge !== "string") return res.writeHead(400).send(); 
            ...
            switch (mode as Mode | "denied") {
                case "subscribe": {
                    const pending = this.pending.find(p => p.mode === mode && p.channelId === channelId) as PendingSubscription<"subscribe">;
                    if (!pending) break;
                ...
                case "unsubscribe": {
                    const pending = this.pending.find(p => p.mode === mode && p.channelId === channelId) as PendingSubscription<"unsubscribe">;
                    if (!pending) break;
                    pending.resolve();
                    break;
                }
                ...
                case "denied": {
                    const pending = this.pending.filter(p => p.channelId === channelId);
    ...
    public getPostListener(): Callback {
        return (req: ExpressRequest, res: ExpressResponse) => {
            ...
            const {valid, status} = verifyPostMessage(body, hubSignature, link, this.secret);
     ...
     * @returns the lease
     */
    public await<TMode extends Mode>(mode: TMode, channelId: string): Promise<TMode extends "subscribe" ? number : void> {
...
export class PubsubClient extends EventEmitter {
    ...
    public async subscribe(channelId: string): Promise<void> {
        ...
        if (this.leases.has(channelId)) clearTimeout(this.leases.get(channelId));
        this.leases.set(channelId, setTimeout(() => this.subscribe(channelId), lease * 1000));
    }
